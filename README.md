...
![Vercel Serverless](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)
![Groq / Llama 3.1](https://img.shields.io/badge/Llama_3.1_(AI)-0466C8?style=for-the-badge&logo=meta&logoColor=white)

O **Valorize** nasceu com um propósito educacional: democratizar o entendimento financeiro. Ao longo do seu desenvolvimento, o projeto evoluiu e pivotou para se tornar um **Smart Dashboard Financeiro Full-Stack** completo, permitindo que os usuários gerenciem seu fluxo de caixa, renda fixa e criptoativos em um único ecossistema seguro, além de contar com um assistente de Inteligência Artificial integrado.

🌐 **[Acesse o projeto ao vivo aqui](https://valorize-kappa.vercel.app)**
...
* **Assistente Inteligente Integrado:** Um chat interativo alimentado pelo modelo **Llama 3.1** (via API da Groq), projetado para atuar como um conselheiro financeiro educacional.
* **Respostas em Tempo Real:** Capacidade de explicar conceitos complexos sobre Renda Fixa, CDBs, Tesouro Direto e gestão de portfólio de forma clara e objetiva.
...
Este projeto foi construído **sem o uso de frameworks no Front-end (Vanilla JS)** para consolidar os fundamentos da web, aliado a uma arquitetura moderna de Back-end na nuvem.

1. **Micro-Backend Serverless (Node.js / Vercel):**
   * Para implementar o Agente Financeiro IA sem expor credenciais sensíveis (API Keys) no lado do cliente, o projeto utiliza uma arquitetura *Serverless* hospedada na Vercel. O Front-end se comunica com funções isoladas em Node.js (`/api/chat`), que fazem a ponte segura com as APIs externas de inteligência artificial, demonstrando boas práticas de segurança e engenharia corporativa.
2. **Gráficos Nativos (Canvas API):**
...
3. **Privacidade por Design (Privacy-First Data):**
   * Em relação ao armazenamento dos dados financeiros do usuário, o sistema opera de forma local. Todos os dados são associados a IDs únicos (UUIDs) e persistidos exclusivamente no `localStorage` do navegador. Nenhum dado financeiro trafega pela rede, garantindo 100% de privacidade ao usuário sobre sua carteira.
4. **UI/UX e Design System:**
   * Interface moderna baseada em tokens CSS (CSS Variables), garantindo consistência visual (cores, espaçamentos, tipografia) e facilitando a implementação futura de temas alternativos. Layout totalmente responsivo utilizando CSS Grid e Flexbox.

---

## 👨‍💻 Sobre o Autor
Desenvolvido por Bruno Arial Ramos, estudante de Engenharia da Computação.
Este projeto reflete minha paixão por criar interfaces limpas e arquiteturas de software eficientes, unindo o desenvolvimento Front-end ao consumo inteligente de APIs, matemática financeira, estruturas de dados no lado do cliente e integração segura com Inteligência Artificial.
